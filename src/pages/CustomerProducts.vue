    ...
    <div class="col-12 q-ma-sm">
      ...
      <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            ...
            <q-btn dense round flat color="grey" icon="info" :to="{ name: 'customer-product-detail', params: { product_id: props.row._id, uid: id }}"></q-btn> 
      ...
      <q-card>
        ...
        <q-card-section>
          <q-form class="row full-width">
            ...
            <q-input filled v-model="item.amount" type="number" hint="Кол-во" label="Кол-во" class="col-6 q-pa-sm q-mb-xl" :suffix="suffixReturn" min="0" :max="item.maxAmount"/>
...
export default {
    ...
    watch: {
      'item.name':{
        handler(value){
          if(value != null && this.products.length != 0){
            let chosenProduct = this.products.filter((element) => element.name == value)
            Object.assign(this.item, {barcode: chosenProduct[0].barcode, name: chosenProduct[0].name, amount: 0, buyPrice: chosenProduct[0].buyPrice,
             sellPrice: chosenProduct[0].sellPrice, description: chosenProduct[0].description, maxAmount: chosenProduct[0].amount},)
          }
        },deep: true
      },
      'item.amount':{
        handler(value){
          if((value != null) && (value > this.item.maxAmount)){
            this.item.amount = 0;
          }
        },deep: true
      },
    ...
    methods: {
      ...mapActions([
          'GET_ALL_PRODUCTS', 'ADD_CUSTOMER_PRODUCT', 'DELETE_PRODUCT_OF_CUSTOMERS', 'EDIT_PRODUCT_OF_CUSTOMER', 'FILTER_PRODUCTS_OF_PRODUCERS_BY_NAME', 'GET_PRODUCTS_OF_CUSTOMERS'
      ]),
      ...
      async editRow(props){
        ...
        this.item = {barcode: null, name: '', amount: null, buyPrice: null, sellPrice: null, description: ''};
