  ...
  <q-page>
    <div class="row">
      <div class="col-12 q-pa-sm">
        <q-btn icon="arrow_back" class="bg-white" outline style="color: grey;" :to="`/producer/products/${uid}`"></q-btn>
        <q-btn
          v-if="$q.screen.gt.xs"
          outline
          dense
    ...
    <div class="col-12 q-ma-sm">
      <q-list bordered separator dense class="bg-white text-green-9 text-subtitle1">
        ...
        <q-item>
          <q-item-section>
            <div>
              <span>Общее кол-во остатка: </span> 
    ...
    <div class="col-12 q-ma-sm">
      ...
      <template v-slot:top="props">
        <q-icon name="shop" class="text-grey-7 text-h4"></q-icon>
        <span class="text-h6 q-ml-sm text-grey-9 text-weight-bold">Продукт {{product.name}}</span>
        <q-space />
      ...
      <q-card>
        ...
        <q-card-section>
          <q-form class="row full-width">
           ...
           <q-input filled v-model="item.date" class="col-6 q-pa-sm">
              <template v-slot:prepend>
                <q-icon name="event" class="cursor-pointer">
                  <q-popup-proxy transition-show="scale" transition-hide="scale">
                    <q-date v-model="item.date" mask="YYYY-MM-DD HH:mm" today-btn>
              ...
              <template v-slot:append>
                <q-icon name="access_time" class="cursor-pointer">
                  <q-popup-proxy transition-show="scale" transition-hide="scale">
                    <q-time v-model="item.date" mask="YYYY-MM-DD HH:mm" now-btn>
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="Применить" color="primary" flat />
                      </div>
                    </q-time>
            ...
            <q-input filled v-model.lazy="item.unPayed" label="Неоплачено" suffix="сум" class="col-6 q-pa-sm"/>
      ...
      <q-card>
        ...
        <q-card-section>
          <q-form class="row full-width">
            ...
            <q-input filled v-model="item.amount" type="number" label="Кол-во (Приход)" class="col-6 q-pa-sm"/>
            <q-input filled v-model="item.amountLeft" type="number" label="Кол-во (Остаток)" class="col-6 q-pa-sm"/>
            <q-input filled v-model="item.buyPrice" suffix="сум" label="Цена покупки" class="col-6 q-pa-sm"/>
            <q-input filled v-model="item.sellPrice" suffix="сум" label="Цена продажи" class="col-6 q-pa-sm"/>
...
export default {
    ...
    data() {
        return {
          ...
          dialogDelete: false,
          error: false,
          item: {date: '', amount: '', amountLeft: '', buyPrice: '', sellPrice: '',
           payed: '', unPayed: '', total: '', about: ''},
          columns: [
            { name: 'index', align: 'center', label: 'No#', field: 'index', sortable: true },
            { name: 'date', align: 'center', label: 'Дата', field: 'date', sortable: true },
            { name: 'amount', align: 'center', label: 'Приход', field: 'amount', sortable: true },
            { name: 'amountLeft', align: 'center', label: 'Остаток', field: 'amountLeft', sortable: true },
    ...
    watch: {
      'item.amount': {
        async handler(value){
          if(value != null){
            if(this.item.amount != '' && this.item.buyPrice != ''){
    ...
    methods: {
      ...mapActions([
          'GET_PRODUCTS_DETAIL_OF_PRODUCER', 'ADD_PRODUCER_PRODUCT_DETAIL', 'DELETE_PRODUCT_DETAIL', 'EDIT_PRODUCT_DETAIL'
      ]),
      async refresh(){
        await this.GET_PRODUCTS_DETAIL_OF_PRODUCER(this.product._id);
        this.data = await this.getProductDetailOfProducer
      },
      dialogInputsChange(){
      ...
      async addProductToDB() {
        if(this.item.date == '' || this.item.amount == '' || this.item.buyPrice == '' || 
        this.item.sellPrice == '' || this.item.payed == '' || this.item.unPayed == ''){
      ...
      async editRow(props){
        ...
        await this.cleanItem();
        let buyPrice = props.buyPrice.replace(/[^0-9]/g,'');
        let sellPrice = props.sellPrice.replace(/[^0-9]/g,'');
        let payed = props.payed.replace(/[^0-9]/g,'');
        let unPayed = props.unPayed.replace(/[^0-9]/g,'');
      ...
      async editProduct(){
        if(Number(this.item.amountLeft) > Number(this.item.amount)){
          this.error = !this.error
        }else{
          await this.EDIT_PRODUCT_DETAIL(this.item)
          await this.refresh();
          this.dialogEdit = !this.dialogEdit
        }
      },
      async deleteRow(props){
        ...
        this.item = {};
        this.item = Object.assign({ did: props._id, pid: this.product._id})
      },
      async deleteProduct(){
        await this.DELETE_PRODUCT_DETAIL(this.item)
        await this.refresh();
